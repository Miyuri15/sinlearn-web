...
export default function ChatPage({
  ...
  const ensureSessionId = useCallback(async (): Promise<string | null> => {
    ...
    try {
      ...
      setToastMessage(
        apiErrorMessage(error, "errors.create_chat_session"),
      );
      setToastType("error");
  ...
  const handleVoiceSend = async (audioBlob: Blob) => {
    ...
    try {
      ...
      try {
        ...
        setToastMessage(
          apiErrorMessage(qaError, "errors.voice_answer"),
        );
        setToastType("error");
        setIsToastVisible(true);
      }
    ...
    if (isClearing) {
      if (activeSessionId) {
        try {
          ...
          setToastMessage(
            apiErrorMessage(e, "errors.remove_rubric"),
          );
          setToastType("error");
          setIsToastVisible(true);
          return;
  ...
  const handleFileUpload = async (files: File[]) => {
    ...
    try {
      ...
      setToastMessage(
        apiErrorMessage(error, "errors.upload_answer_sheets"),
      );
      setToastType("error");
      setIsToastVisible(true);
    } finally {
  ...
  const handleRemoveEvaluationFile = async (index: number) => {
    ...
    try {
      ...
      setToastMessage(
        apiErrorMessage(e, "errors.remove_answer_sheet"),
      );
      setToastType("error");
      setIsToastVisible(true);
      return;
  ...
  const handleReplaceEvaluationFile = async (index: number, file: File) => {
    ...
    try {
      ...
      setToastMessage(
        apiErrorMessage(error, "errors.replace_answer_sheet"),
      );
      setToastType("error");
      setIsToastVisible(true);
    } finally {
  ...
  const handleProcessEvaluation = async () => {
    ...
    try {
      ...
      setProcessingStatus("idle");
      setToastMessage(
        apiErrorMessage(error, "errors.process_documents"),
      );
      setToastType("error");
      setIsToastVisible(true);
    } finally {
  ...
  const handleOpenMarkingSchema = useCallback(async () => {
    ...
    try {
      ...
      setToastMessage(
        apiErrorMessage(error, "errors.load_marking_schema"),
      );
      setToastType("error");
      setIsToastVisible(true);
    } finally {
  ...
  const handleNewChat = async (mode: "learning" | "evaluation") => {
    ...
    try {
      ...
      setToastMessage(
        apiErrorMessage(error, "errors.open_new_chat"),
      );
      setToastType("error");
      setIsToastVisible(true);
    }
  ...
  const handleConfirmEdit = () => {
    ...
    const run = async () => {
      try {
        ...
        setToastMessage(
          apiErrorMessage(error, "errors.update_chat_title"),
        );
        setToastType("error");
        setIsToastVisible(true);
      }
  ...
  const handleConfirmDelete = () => {
    ...
    const run = async () => {
      ...
      try {
        ...
        setToastMessage(
          apiErrorMessage(error, "errors.delete_chat"),
        );
        setToastType("error");
        setIsToastVisible(true);
      } finally {
        ...
        <EvaluationMarksModal
          ...
          onSubmit={async (config) => {
            ...
            try {
              ...
              setToastMessage(apiErrorMessage(e, "errors.confirm_paper_config"));
              setToastType("error");
              setIsToastVisible(true);
              throw e;
            }
