...
const QuestionsPanelpage = ({ onClose }: QuestionsPanelProps) => {
    ...
    const [uploadedQuestion, setUploadedQuestion] = useState<QuestionItemType | null>(null);
        ...
        if (files && files.length > 0) {
            ...
            if (fileExt && validExtensions.includes(fileExt)) {
                ...
                const newQuestion: QuestionItemType = {
                    ...
                    details: `File: ${fileExt.toUpperCase()} | Size: ${(file.size / 1024 / 1024).toFixed(2)} MB`,
                    fileUrl: fileUrl,
                    fileType: fileExt
                };
    ...
    const handleViewQuestion = () => {
        if (uploadedQuestion && uploadedQuestion.fileType === 'pdf' && uploadedQuestion.fileUrl) {
            setViewingPDF({
            ...
            showToast(t('preview_not_available', { fileType: uploadedQuestion.fileType?.toUpperCase() }), 'error');
        } else {
      ...
      <div className="h-full w-[400px] bg-white dark:bg-[#111111] border-l border-gray-200 dark:border-[#2a2a2a] flex flex-col p-0">
       ...
       <div className="p-6 grow overflow-y-scroll hidden-scrollbar">
          ...
          <div>
            ...
            {!uploadedQuestion && (
              <p className="text-sm text-gray-400 italic mt-4">{t('no_questions')}</p>
            )}
